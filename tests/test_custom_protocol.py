...
def test_encode_string_message(protocol):
    ...
    # Check header format
    total_length = struct.unpack('!I', message[4:8])[0]  # Length moved to bytes 4-8
    command = struct.unpack('!H', message[2:4])[0]       # Command moved to bytes 2-4
    assert command == cmd
...
def test_decode_message(protocol):
    ...
    _, _, decoded_cmd, total_length, payload = protocol.decode_message(message)
    assert total_length == len(message)
...
def test_encode_decode_string(protocol):
    ...
    _, _, _, _, encoded = protocol.decode_message(payload)
...
def test_encode_decode_integer_list(protocol):
    ...
    _, _, _, _, payload = protocol.decode_message(message)
...
def test_decode_success_response(protocol):
    ...
    _, _, _, _, payload = protocol.decode_message(success_message)
...
def test_encode_empty_payload(protocol):
    ...
    _, _, cmd, total_length, payload = protocol.decode_message(message)
    assert total_length == len(message)
