...
## Comparison of gRPC vs JSON-over-Sockets vs Custom Protocol
...
| **gRPC**  | Protobuf serialized `Login` message with gRPC framing header        | 22 | - Protobuf payload:<br/>&emsp;• Username: key (1) + length (1) + "alice" (5) = 7B<br/>&emsp;• Password: key (1) + length (1) + "Pass1234" (8) = 10B<br/>- Total payload = 17B + 5B gRPC header  |
